...
public class DataSourceConfig {
  public static final MigrationVersion LATEST_STABLE_VERSION = MigrationVersion.fromVersion(
    "24"
  );
  ...
  public DataSource getDataSource(LavagnaEnvironment env)
    throws URISyntaxException {
    HikariDataSource dataSource = new HikariDataSource();
    dataSource.setDriverClassName(
      getDriveClassName(env.getRequiredProperty("datasource.dialect"))
    );
    ...
    if (System.getProperty("startDBManager") != null) {
      ...
      DatabaseManagerSwing.main(
        new String[] { "--url", "jdbc:hsqldb:mem:lavagna", "--noexit" }
      );
    }
   ...
   * @throws URISyntaxException
   */
  private static void urlWithCredentials(
  ...
  private static String scheme(URI uri) {
    return "postgres".equals(uri.getScheme())
      ? "jdbc:postgresql"
      : uri.getScheme();
  }

  private static void urlAndCredentials(
    HikariDataSource dataSource,
    LavagnaEnvironment env
  ) {
    ...
    dataSource.setPassword(
      env.getProperty("datasource.password") != null
